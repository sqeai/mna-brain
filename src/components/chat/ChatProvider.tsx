...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  // Persist on every messages change (including during streaming)
  useEffect(() => {
    if (messages.length > 0) saveMessagesToStorage(messages);
    else if (typeof window !== "undefined") localStorage.removeItem(STORAGE_KEY);
  }, [messages]);

  // Keep a ref so pagehide/visibility handlers always save latest messages (avoid stale closure)
  const messagesRef = useRef(messages);
  messagesRef.current = messages;

  // Persist when user leaves the page (tab switch, navigate away, close) so partial stream is saved
  useEffect(() => {
    const save = () => {
